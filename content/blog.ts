...
// `updated`, `draft`, `discuss`, `image`) and its media in `content/blog/<slug>/`. The body is
// the markdown as written; the rewrites below are mechanical and happen here, never in the file.
...
import { slugger } from '@aweftjs/ui';
 ...
 *
 * Throws: `PostError` for a file with no front matter, no title, no date, a date that does not
 * parse, or a published post with no description.
 */
export const readPost = (file: string): Source => {
	...
	if (!isDate(date)) throw new PostError(file, `the date does not parse: ${date}`);
	const updated = text(front, 'updated');
	if (updated !== undefined && !isDate(updated)) throw new PostError(file, `the updated date does not parse: ${updated}`);
	const draft = front['draft'] === true;
...
/**
 * Every heading of a body with the id the page gives it: `Markdown` runs `slugger` over the same
 * headings in the same order, so a contents list built from these points where the headings are.
 */
export const headings = (markdown: string): Heading[] => {
	const id = slugger();
	const found: Heading[] = [];
	for (const run of runs(markdown)) {
		if (run.kind !== 'prose') continue;
		for (const line of run.text.split('\n')) {
			...
			found.push({ level: match[1]!.length, text: stripInline(raw), id: id(raw) });
		}
	}
	return found;
};
...
const mediaPath = (folder: string, src: string): string => join(folder, src.replace(/^\.?\//, ''));
 ...
 *
 * A draft is read and checked, and its missing media is a warning; it is not in the index, its
 * media is not published and its twin is not written. A published post with a missing image, or
 * a poster that cannot be fetched, fails the build with the reason.
 *
...
export const buildBlog = async (options: BuildOptions): Promise<Built> => {
	...
	for (const source of sources) {
		...
		let body = inlineReferences(source.body);
